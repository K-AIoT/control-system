...
void loop() {
  ...
  now1 = millis();
  now2 = millis();
  if (now1 - lastMeasure1 > 200) {  // checks measurement interval
    lastMeasure1 = now1;
    digitalVoltage = analogRead(analogPin);                          //sets up measurement interval
    client.publish("dry/feedback", String(digitalVoltage).c_str());  // publishes humidity
    Serial.print("Measured voltage: ");
    Serial.println(digitalVoltage);
    if (now2 - lastMeasure2 > 2000) {  // checks measurement interval
      lastMeasure2 = now2;
